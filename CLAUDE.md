...
- `js/main.js` — DOM制御とゲーム進行。**ソロ／オンラインの2モード**を持ち、`applyView()` が画面（ソロ盤面／オンライン作成画面／対戦画面）の表示を一元制御する。ソロは `state`（`words`/`used`/`over`）を保持。オンラインは `session`（code/playerId/seat）＋ルーム購読で描画。描画関数（renderBoard/renderChain等）は両モードで共通化。DOMアクセスはこのファイルに閉じる。
...
- `js/history.js` — 過去ゲーム結果の localStorage 永続化（`loadHistory`/`saveGame`/`clearHistory`、最大20件・新しい順）。※現状ソロのみ保存。
...
`judge()` の戻り値 `{ok, reason?, end?}` が状態遷移の中心。`end` は `"lose"`（「ん」終了・重複）を表す。文字数しばり等の違反は `ok:false`（`end`なし）の再入力要求。新ルール追加時はこの契約を保つこと（online.jsの権威判定も同じ`judge()`を通すため自動で整合する）。
